...
def brute_force(cipher_text):
    ...
    for length in range(1, len(cipher_text)):
        ...
        for p in permutation_list:
            ...
            # has to have 80% of words from dictionary
            threshold = 80
            if getEnglishCount(plain_text) > threshold:
                percentages[str(p)] = getEnglishCount(plain_text)
    
    # cleans up the string to become a list 
    key_break = findMaxInd(percentages).replace(",", "").replace("(", "").replace(")","").split()
    
    key_break = list(map(int, key_break))
    # v v v optional way to turn all strings in the list into integers
    # key_break = [int(i) for i in key_break]
...
def main():
    ...
    if choice == "e":
        ...
        if hack == "k": 
            ...
            plain_text = brute_force(cipher_text)
